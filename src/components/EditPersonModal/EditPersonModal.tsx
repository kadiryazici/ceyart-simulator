...
export type EditPersonModalProps = ComponentProps<"div"> & {
  person: Person
  onClose: () => void
}

const genderOptions: Gender[] = ["Erkek", "Kadın", "Bilinmiyor"]

export function EditPersonModal(props: EditPersonModalProps) {
  const { person, onClose, className, ...attrs } = props
  const updatePerson = useSimulatorStore((state) => state.updatePerson)
  const [number, setNumber] = useState(String(person.number))
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    updatePerson(person.id, { number: cleanNumber, name, gender })
    onClose()
  }
    ...
    >
      <form className="grid w-full max-w-sm gap-4 rounded-[28px] border border-slate-950/10 bg-white/95 p-5 shadow-2xl shadow-slate-950/20 backdrop-blur-xl" onSubmit={handleSubmit} onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between gap-3">
          <h2 id="edit-person-title" className="text-lg font-[760] text-slate-900">Kişiyi düzenle</h2>
